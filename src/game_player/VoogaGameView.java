...
public class VoogaGameView implements GameView {
	...
	public VoogaGameView(List<Level> gameLevels) {
		...
		myPlayer = new Player();
		myControls = new Controls(myPlayer);
		myGameLevels.get(myCurrLevel).addObject(myPlayer);
		initListeners();
	}
	...
	private void initListeners() {
		...
		myGame.addEventHandler(KeyEvent.KEY_PRESSED, (key) -> {
	...
	private void step(double elapsedTime) {
		if (myGameStatus) {
			...
			myGame.getChildren().add(myGameLevels.get(myCurrLevel).getGrid());
		}
	}
	...
	public void updateGame() {
		...
		if (temp % 2 == 0) {
			...
			myGame.getChildren().add(addRect);
		} else {
